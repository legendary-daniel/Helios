...
#property description "Helios ML Trading Expert Advisor - Python Integration Fixed"
#property description "Full Python communication with guaranteed compilation"
...
void OnTick()
{
    ...
    if(EnablePythonComm && pythonConnected && (TimeCurrent() - lastHeartbeat) > HeartbeatInterval)
    {
        SendHeartbeat();
    }
...
bool InitializePythonCommunication()
{
    ...
    // In production, you would implement actual HTTP/WebSocket connection here
...
void ProcessTradingSignal(TradingSignal& signal)
{
    ...
    string resultData = "{\"success\":" + (result ? "true" : "false") + 
                       ",\"symbol\":\"" + signal.symbol + 
                       "\",\"signal_type\":" + IntegerToString(signal.signalType) + "}";
...
void SimulateReceivedSignal()
{
    ...
    int symbolCount = 4; // Fixed count to avoid ArraySize compatibility issues
    string symbol = symbols[MathRand() % symbolCount];
...
void ManageOpenPositions()
{
    for(int i = PositionsTotal() - 1; i >= 0; i--)
    {
        if(position.SelectByIndex(i))
        {
            ...
            if(profit > AccountInfoDouble(ACCOUNT_BALANCE) * 0.02)
            {
                shouldClose = true;
            ...
            if(profit < -AccountInfoDouble(ACCOUNT_BALANCE) * 0.01)
            {
                shouldClose = true;
...
bool ClosePosition(string symbol, string reason)
{
    ...
    if(result)
    {
        ...
        string closeData = "{\"symbol\":\"" + symbol + 
                          "\",\"reason\":\"" + reason + 
...
void UpdateAccountInfo()
{
    ...
    // Account info automatically refreshed
...
double CalculateDailyPnL()
{
    ...
    // Add unrealized P&L from open positions (simplified approach)
...
{
    string status = "{";
    status += "\"ea_running\":true,";
    status += "\"account_balance\":" + DoubleToString(AccountInfoDouble(ACCOUNT_BALANCE), 2) + ",";
    status += "\"account_equity\":" + DoubleToString(AccountInfoDouble(ACCOUNT_EQUITY), 2) + ",";
    ...
    status += "\"python_connected\":" + (pythonConnected ? "true" : "false") + ",";
    status += "\"manual_mode\":" + (manualMode ? "true" : "false");
    status += "}";
...
bool IsTradeAllowed()
{
    ...
    if(AccountInfoInteger(ACCOUNT_TRADE_MODE) != ACCOUNT_TRADE_MODE_FULL)
    {
...
bool CheckRiskManagement(TradingSignal& signal)
{
    ...
    if(MathAbs(dailyPnL) > AccountInfoDouble(ACCOUNT_BALANCE) * MaxDailyLoss / 100)
    {
...
void OnTimer()
{
    // Simple timer function to avoid compilation issues
    // Periodic status updates every 10 minutes
    static datetime lastTimerUpdate = 0;
    if(TimeCurrent() - lastTimerUpdate > 600) // Every 10 minutes
    {
